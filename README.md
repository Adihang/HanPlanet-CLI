...
```bash
uv run hanharness         # 개발 환경 (TUI)
hanharness                # pipx 전역 설치 후
hanharness -p "질문"      # 비대화형 모드
```
...
시작 시 표시되는 ASCII 배너를 HANHARNESS 6행 그라데이션 아트로 교체, 하단에 `www.hanplanet.com` 표기.
...
| `pyproject.toml` | `[project.scripts]` 의 `hanharness` entry point |
...
| `WelcomeBanner.tsx` | HANHARNESS 그라데이션 ASCII 배너 |
...
`oh`/`hanharness`와 독립된 별도 CLI. 채팅앱 채널에서 항상 작동하는 개인 에이전트를 구성.
...
```toml
[project.scripts]
hanharness = "openharness.cli:app"  # 커스텀 진입점
